	...
	<view>
		...
		<view  class="body after-navber pr">
		    <view class="status-bar" :style="(order_refund.refund_status==1 || order_refund.refund_status==0)?'background: #3DBB12;':'background: #FF401A'">
		    	...
		    	<view>
		    		<block v-if="order_refund.is_delete==1">
		    			<view>售后申请已取消</view>
		    		</block>
		    		<block v-else>
		    			<!-- 退货 -->
		    			<block v-if="order_refund.refund_type==1">
		    				...
		    				<block style="color:#36ba75" v-if="order_refund.refund_status==1">审核通过，货款即将到账，如有疑问请联系客服</block>
		    				<block style="color:#c10000" v-if="order_refund.refund_status==3">已拒绝退货</block>
		    			</block>
		    			
		    			<!-- 退款 -->
		    			<block v-if="order_refund.refund_type==3">
		    				<block v-if="order_refund.refund_status==0">等待审核中</block>
		    				<block style="color:#36ba75" v-if="order_refund.refund_status==1">审核通过，退款即将到账，如有疑问请联系客服</block>
		    				<!-- <block style="color:#36ba75" wx:if="{{order_refund.is_agree==1 && order_refund.refund_status==2}}">已同意退款,等待银行处理</block> -->
		    				<block style="color:#c10000" v-if="order_refund.refund_status==3">已拒绝退款</block>
		    			</block>
		    		</block>
		    				...
		    				<view class="flex-grow-1 flex-col">
		    					...
		    					<view class="flex-grow-0 flex-row" style="font-size: 24rpx">
		    						<view class="color-999999">
		    							<view v-for="(item, index) in order_refund.attr" :key="index" style="margin-right: 40rpx;display: inline-block">
		...
		<view class="footer flex justify-end">
			...
			<view class="btn btn-red-full" @tap="cancelRefund" v-if="order_refund.is_delete==0 && order_refund.refund_status==0">撤销申请</view>
			<button class="btn btn-red" open-type="contact" :session-from="'refund_id=' + order_refund_id + '|nickName=' + nickName + '|avatarUrl=' + avatarUrl + '|type=' + type + '|userId=' + userId + '|mchId=' + mchId + '|storeId=' + storeId" @contact="handleContact">联系客服</button>
		</view>
		<!-- 固定按钮结束 -->
	</view>
</template>
...
export default {
  ...
  onLoad: function (options) {
    ...
    self.getApp().request({
      url: self.getApp().api.order.refund_detail,
      data: {
        order_refund_id: options.id
      },
