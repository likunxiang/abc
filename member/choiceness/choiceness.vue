<template>
	<view>
		...
		<custom lineColor="#dcdcdc" borderColor="rgba(220,220,220,0.6)" btnMode="2" backColor="#000000" houseBk="true" mode="2"
		 fixed="fixed" title="精选推荐" titleColor="#000000" :background="'#FFFFFF'"></custom>

		<view class="hot_tab" v-if="platform_navs.length>0" :style="{'top': customBar + 'px'}">
			<text v-for="(item,index) in platform_navs" :key="index" :class="{'active':type == item.sign}" @tap="choose_nav(item.sign)"
			 class="hot_item">{{item.name}}</text>
		</view>

		<!-- tab栏位 -->
		<view v-if="navs.length>0" class="tabs fs-30" style="width: 100%; overflow-x: scroll;padding-left: 24rpx;" :style="{'top': 'calc(' + customBar + 'px' + ' + 80rpx)'}">
		...
		<!-- 排序筛选 -->
		<view class="screen flex align-center justify-between" :style="{'top': 'calc(' + customBar + 'px' + ' + 157rpx)'}">
			<view :class="default_sort?'color-EB0401':''" @click="defaultSort">默认排序</view>
			<view class="flex align-center">
				<view class="flex align-center" :class="!default_sort&&arr[0]>0?'color-EB0401':''" @click="filter(0)">
					<view>佣金比例</view>
					<view v-if="arr[0]== 0" class="iconfont icon-paixu" style="font-size: 28rpx;"></view>
					<view v-else-if="arr[0]==1" class="iconfont icon-shengxu"></view>
					<view v-else-if="arr[0]==2" class="iconfont icon-asc"></view>
				</view>
				<view class="flex align-center ml-24" :class="!default_sort&&arr[1]>0?'color-EB0401':''" @click="filter(1)">
					<view>价格</view>
					<view v-if="arr[1]==0" class="iconfont icon-paixu" style="font-size: 28rpx;"></view>
					<view v-else-if="arr[1]==1" class="iconfont icon-shengxu"></view>
					<view v-else-if="arr[1]==2" class="iconfont icon-asc"></view>
				</view>
				<view class="flex align-center ml-24" :class="!default_sort&&arr[2]>0?'color-EB0401':''" @click="filter(2)">
					<view>销量</view>
					<view v-if="arr[2]== 0" class="iconfont icon-paixu" style="font-size: 28rpx;"></view>
					<view v-else-if="arr[2]==1" class="iconfont icon-shengxu"></view>
					<view v-else-if="arr[2]==2" class="iconfont icon-asc"></view>
				</view>
			</view>
		</view>
		<!-- 排序筛选 end -->
	...
	export default {
		...
		methods: {
			...
			filter(index) {
				...
				} else if (this.arr[index] == 1) {
					this.$set(this.arr, index, 2)
				} else if (this.arr[index] == 2) {
					this.$set(this.arr, index, 1)
				}
