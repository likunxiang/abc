<template>
	<view>
		...
		<custom lineColor="#dcdcdc" borderColor="rgba(220,220,220,0.6)" btnMode="2" backColor="#000000" houseBk="true" mode="2"
		 fixed="fixed" title="超市精选" titleColor="#005720" background="#FFFFFF"></custom>

		<view class="flex flex-y-center pt-20 pl-24 pr-24 color-fff" style="background-color: #FFFFFF;">
			<!-- <view :class="{'active':type == 'jingdong'}" hover-class="click" class="flex align-center flex-x-center fs-30" style="height: 70rpx;border-radius: 8rpx;background-color: #F3F3F3;width:160rpx;color:#595959;font-weight: bold;">
				<text @tap="choose_nav('jingdong')">京东馆</text>
			</view>
			<view :class="{'active':type == 'taobao'}" hover-class="click" class="flex align-center flex-x-center  fs-30 ml-24" style="height: 70rpx;border-radius: 8rpx;background-color: #F3F3F3;width:160rpx;color:#595959;font-weight: bold;">
				<text @tap="choose_nav('taobao')">天猫馆</text>
			</view> -->
			<view class="flex flex-1 flex-y-center" style="height: 70rpx;background-color: #F4F4F4;border-radius: 8rpx;">
				...
				<input v-if="type == 'jingdong'" @blur="seach" placeholder="输入商品关键字" type="text" v-model="keyword" class="flex-1 pl-24 pr-24" style="height: 70rpx;font-size: 30rpx;color: #333333;" />
				<input @tap.stop="less(toPage,'/pages/seachAll/seachAll?type=3&keyword='+keyword)" v-else disabled placeholder="输入商品关键字" type="text" v-model="keyword" class="flex-1 pl-24 pr-24" style="height: 70rpx;font-size: 30rpx;color: #333333;" />
			</view>
		</view>
		...
		<view class="goods_9 flex flex-wrap flex-x-b pl-24 pr-24">
			<view class="goods_9_item" v-for="(item,index) in goods" :key="index" @tap.stop="less(toPage,'/pages/goodsTK/goodsTK?id=' + item.goods_id + '&type=' + type)" hover-class="click1">
				...
				<view class="flex flex-x-b flex-y-end goods_9_price">
					<view class="left flex flex-y-end goods_9_priceall">
						...
						<view class="quan" v-if="item.discount_price">
							券{{item.discount_price || '0'}}元
						</view>
					</view>
					<view class="right">
