...
<view class="page">
	<!-- 取消申请功能 -->
	<view class="cancel-model flex-x-center flex-y-center" v-if="is_cancel">
	...
	<block v-else>
		<view class="body">
			<view class="order-box bg-white plr-25 mb-20 pr" v-for="(order, index) in orderList" :key="index">
				...
				<navigator :url="'/private/privateOrderdetail/privateOrderdetail?order_id=' + order.id" class="order-list-no-pay-body border-bottom-dotted">
					<view class="flex mb-20" v-for="(goods, index) in order.mchMyOrderDetail" :key="index">
				...
				<view class="order-list-no-pay-btn flex justify-between">
					<view>
						<view class="color-489A19 fs-24" v-if="order.is_send ==1 && order.is_confirm == 0 && order.is_cancel == 0">等待顾客确认收货</view>
					</view>
					<view class="flex">
						<view class="order-list-btn order-list-btn-yellow" v-if="order.apply_delete == 1 && order.is_cancel == 0" :data-id="order.id" @tap="orderAudit">待审核</view>
						<view class="order-list-btn" v-if="order.is_cancel_show" :data-id="order.id" :data-index="index" @tap="orderRevoke">取消订单</view>
						<navigator :url="'/pages/express-detail/express-detail?id=' + order.id + '&type=private'" class="order-list-btn" v-if="order.is_expressNo_show">查看物流</navigator>
						<navigator :url="'/private/privateDistribution/privateDistribution?id=' + order.id" class="order-list-btn" v-if="order.is_send_show && order.is_send_show == 'wl'" :data-id="order.id" :data-index="index">配送发货</navigator>
						<view class="order-list-btn" v-if="order.is_send_show && order.is_send_show == 'zt'" :data-id="order.id" :data-index="index" @tap="pickUpSend">自提发货</view>
					</view>
				</view>
			</view>
		</view>
...
export default {
  ...
  methods: {
    ...
    getOrderList() {
      ...
      this.getApp().request({
        url: this.getApp().api.selfSupport.get_order_list,
        data: {
          userId: this.getApp().core.getStorageSync('USER_INFO').id,
          now_status: this.status || '1',
          search_name: this.seachValue || ''
        },
        success: res => {
          if (res.code == 0) {
            this.setData({
    ...
    orderRevoke(e) {
      ...
      wx.showModal({
        ...
        success: res => {
          if (res.confirm) {
            this.getApp().request({
              url: this.getApp().api.selfSupport.cancel_order_shoper,
              data: {
                userId: this.getApp().core.getStorageSync('USER_INFO').id,
                id: id
              },
    ...
    orderAudit(e) {
      ...
      wx.showModal({
        ...
        success: res => {
          if (res.confirm) {
            this.getApp().request({
              url: this.getApp().api.selfSupport.audit_order_cancel,
              data: {
                userId: this.getApp().core.getStorageSync('USER_INFO').id,
                is_cancel: 1,
                // 1同意，0驳回
