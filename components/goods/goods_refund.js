module.exports = {
	...
	descInput: function(e) {
		...
		var refund_data = self.refund_data;
	...
	chooseImage: function(e) {
		...
		uni.chooseImage({
			...
			success: function(res) {
				...
				self.setData({
					refund_data: JSON.parse(JSON.stringify(refund_data))
				});
			}
		});
	},
	...
	refundSubmit: function(e) {
		...
		// 	return
		// }
		if(self.refund_type_id) {
			var type = self.refund_type_id
			console.log('提交'+self.type)
		} else {
			uni.showToast({
				title: '请选择退款类型',
				icon: 'none'
		...
		var httpUrl = self.getApp().api.order.refund;
		var navigateToUrl = '';
		var orderType = '';
		...
		/*--退货退款开始--*/
		// if (type == 1) { //退货退款
			var desc = self.refund_data.desc || "";
			...
			//如果有图片先上传图片
			if (self.refund_data.pic_list && self.refund_data.pic_list.length > 0) {
				uni.showLoading({
					title: "正在上传图片",
					mask: true,
				});
				for (var i in self.refund_data.pic_list) {
					(function(i) {
						uni.uploadFile({
							url: self.getApp().api.default.upload_image,
							filePath: self.refund_data.pic_list[i],
							name: "image",
							success: function(res) {
								pic_complete_count++;
								...
								if (pic_complete_count == self.refund_data.pic_list.length) {
									uni.hideLoading();
									_submit();
								}
							},
			...
			function _submit() {
				...
				if (type == 1) {
					var data = {
						type: type,
						order_detail_id: self.goods.order_detail_id,
						desc: desc,
						...
						reasons_for_return: self.tuikuangIndex ? self.tuikuangCause[self.tuikuangIndex].id : '',
						packageDesc: self.packagingIns,
						isHasPackage: self.packageIns,
						isNeedDetectionReport: self.checkIns,
						reason_comp: self.reason_comp,
						refund_num: self.refund_num,
						returnwareType: self.aogIns
					}
				} else {
					var data = {
						type: type,
						...
						orderType: orderType,
						// reasons_for_return: self.data.returnIndex ? self.data.returnCause[self.data.returnIndex].id : '',
						reason_for_return: self.tuikuangIndex ? self.tuikuangCause[self.tuikuangIndex].id : '',
					...
					success: function(res) {
						...
						if (res.code == 0) {
							...
							wx.requestSubscribeMessage({
								tmplIds: [self.refundId, self.sendId],
								success: function(res) {
									//console.log(res);
								},
								complete: function(res) {
