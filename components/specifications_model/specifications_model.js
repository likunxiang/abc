module.exports = {
  ...
  showDialogBtn: function (e) {
    ...
    self.getApp().request({
      url: self.getApp().api.default.goods,
      data: {
        id: current_good.id
      },
  ...
  updateData: function () {
    var self = this.currentPage;
    var currentGood = self.currentGood; //当前商品信息

    var carGoods = self.carGoods; //购物车数据
    ...
    for (var i in attr) {
      ...
      for (var i2 in attr[i].attr_list) {
        arr.push([attr[i].attr_list[i2].attr_id, currentGood.id]);
      }
      ...
      for (var j in carGoods) {
        ...
        for (var j2 in carGoods[j].attr) {
          arr2.push([carGoods[j].attr[j2].attr_id, carGoods[j].goods_id]);
        }
  ...
  checkUpdateData: function (checked_attr) {
    ...
    for (var j in carGoods) {
      ...
      for (var j2 in carGoods[j].attr) {
        arr.push([carGoods[j].attr[j2].attr_id, carGoods[j].goods_id]);
      } // 根据商品ID 和规格ID进行判断
  ...
  attrClick: function (e) {
    ...
    var currentGood = self.currentGood; // 添加选择按钮样式
    ...
    for (var i in attr_group_list) {
      for (var j in attr_group_list[i].attr_list) {
        ...
        if (cAttr.checked === true) {
          checked_attr.push([cAttr.attr_id, currentGood.id]);
        }
      }
    }

    var attr = JSON.parse(currentGood.attr);
    var temporaryGood = self.temporaryGood;

    for (var k in attr) {
      var arr = [];

      for (var k2 in attr[k].attr_list) {
        arr.push([attr[k].attr_list[k2].attr_id, currentGood.id]);
  ...
  checkAttrNum: function () {
    var self = this.currentPage;
    var attrGroupList = self.attr_group_list;
    var attrGroups = JSON.parse(self.currentGood.attr);
    var checkedAttr = self.checked_attr;
    var newAttrArr = [];
    ...
    for (var n in checkedAttr) {
      for (var i in attrGroups) {
        ...
        var inArray = self.getApp().helper.inArray(checkedAttr[n][0], attrArr);
    ...
    for (var w in attrGroupList) {
      for (var w2 in attrGroupList[w].attr_list) {
        ...
        var inArray = self.getApp().helper.inArray(attr.attr_id, newAttrArr);
  ...
  onConfirm: function (e) {
    var self = this.currentPage;
    var attrGroupList = self.attr_group_list;
    var checked_attr = self.checked_attr;
    var current_good = self.currentGood;
    ...
    var check_num = self.check_num ? self.check_num + 1 : 1;
    ...
    for (var i in attrs) {
      ...
      for (var i2 in attrs[i].attr_list) {
        arr.push([attrs[i].attr_list[i2].attr_id, current_good.id]);
    ...
    for (var j in carGoods) {
      ...
      for (var j2 in carGoods[j].attr) {
        cArr.push([carGoods[j].attr[j2].attr_id, carGoods[j].goods_id]);
      } // 如果商品已存在则不需要重复添加
  ...
  guigejian: function (e) {
    ...
    var checked_attr = self.checked_attr;
    var carGoods = self.carGoods;
    var check_num = self.check_num ? --self.check_num : 1;
    var current_good = self.currentGood;
    ...
    for (var i in carGoods) {
      ...
      for (var i2 in carGoods[i].attr) {
        arr.push([carGoods[i].attr[i2].attr_id, carGoods[i].goods_id]);
      }
  ...
  attrGoodStatistics: function () {
    var self = this.currentPage;
    var currentGood = self.currentGood; //当前点击的商品

    var carGoods = self.carGoods;
    var quickList = self.quick_list;
    var quickHot = self.quick_hot_goods_lists;
    var num = 0;
