var area_picker = {
	...
	init: function(args) {
		...
		//默认选择 
		let defaultChecked = args.checked;
		/////////////////////
		var city_list = picker.data[defaultChecked[0]].list || [];
		var district_list = [];
		////////////////////
		if (city_list.length > 0)
			district_list = city_list[defaultChecked[1]].list || [];
		var town_list = [];
		...
		// picker.page.setData({
		// 	old_value: defaultChecked
		// })
		let newArr = defaultChecked.map((val,ind)=>{
				return val;
			})
		picker.page.$nextTick(function(){
			picker.page.old_value = [0,0,0];
			// picker.page.old_value =  newArr;
			picker.page.$nextTick(function(){
				// picker.page.old_value = [0,0,0];
				picker.page.old_value =  newArr;
			})
